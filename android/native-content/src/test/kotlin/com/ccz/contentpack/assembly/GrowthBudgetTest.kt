...
class GrowthBudgetTest {
    ...
    @Test
    fun gradeMulPctClampsOutOfRangeToHighestTier() {
        ...
        assertEquals(150, GrowthBudget.gradeMulPct(9, tiered), "out-of-range grade falls back to the top tier")
    }

    // --- assembly integration: BattleAssembler budgets reserves through the same closed form ---

    private fun unit(id: String, classId: String, level: Int): UnitDef = UnitDef(
        identity = UnitIdentity(id = id, name = id, classId = classId, faction = Faction.PLAYER),
        profile = UnitProfile(level = level, hpMax = 200, stats = base),
    )

...
// Mirrors Combatant's flat 8-field shape; `effects`/`ailments` are new top-level fields (default empty) for
// additive save forward-compat, so grouping isn't an option — suppress the param gate (a wire DTO, not a god-object).
...
internal data class CombatantDto(
    ...
    val ailments: List<ActiveAilmentDto> = emptyList(),
)
...
@Serializable
internal data class ActiveAilmentDto(val kind: String, val remaining: Int)

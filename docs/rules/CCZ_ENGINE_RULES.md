...
- selected S-script pre deployment order（`pre` MoveUnit/RemoveUnit 必须引用已由更早 `pre` SpawnUnit 部署且尚未移除的单位）。`[machine-gated]` `CampaignAssembler` 装配前校验（CampaignAssembler pre-deployment tests）。

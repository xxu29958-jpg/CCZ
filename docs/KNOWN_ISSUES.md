...
| 敌方 AI 侵略式 + 焦点火力 + 支援治疗 | 远程 kiting（#58）+ **目标低血优先**（focus-fire）+ **支援治疗**（ADR 0008：有 heal 技且半血以下友军在射程内 → 治最残友军,support-first;cast/effect 技永不当攻击）已落地。剩：**AI 只 auto-cast heal**（`healCommand` 仅对含 `SkillEffect.Heal` 的技触发）——**不会主动 buff / debuff / 施加 ailment（沉默）**，这些目前只有玩家会施放（敌方持有则 dormant；一个被沉默的敌 healer 取空 cast 目标后正确降级为普攻，不死循环）；**本方单位行动序**仍按 unit id（源游戏：低血单位先动、骑兵先于步兵——需兵种类别/速度模型）；**pure-healer 站位**（只有 heal 技的单位按近敌/Wait 移动,无「贴友军待命」模型——本批敌 healer 程远志兼有普攻故无碍）;无「防守区域 / 仇恨」模式。 | 行动序 + pure-healer 站位 + AI auto-buff/debuff/ailment 启发式 + 仇恨 后续片（源游戏行为记于 `EnemyAi` KDoc）。 |

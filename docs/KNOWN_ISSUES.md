...
| promoted legacy stages 暂用默认玩家三人 + 普攻 | `generateLegacyStage` 已能把 original-ready 旧关卡提升成 native pack 并在写出前 validate+assemble；`legacy_stage_2`「曲阳围城战」已接进 `PromotedStageRuntimes` 可玩。但旧作玩家军来自交互布阵，关卡脚本只给敌/友部署，故通用 generator 目前固定放入刘备/关羽/张飞三人；`LegacySkillMapper` 仍只给全员 `skill_1` 普攻。这是可玩 promotion 的保守默认，不是旧作完整布阵/技能还原。 | 做批量 promotion 前：引入显式 player deployment policy（按剧情/存档/作者配置），并把真技能 importer 扩出普攻以外的效果/战法。 |

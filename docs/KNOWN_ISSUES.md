...
| promoted legacy stages 暂用默认玩家三人 + 普攻 | `generateLegacyStage` 已能把 original-ready / proposal-ready 旧关卡提升成 native pack 并在写出前 validate+assemble；`legacy_stage_1` 到 `legacy_stage_6`、`legacy_stage_8` 到 `legacy_stage_14`、`legacy_stage_16` 到 `legacy_stage_19`、`legacy_stage_21`、`legacy_stage_23` 到 `legacy_stage_24`、`legacy_stage_29` 到 `legacy_stage_31`、`legacy_stage_34` 到 `legacy_stage_42`、`legacy_stage_44` 到 `legacy_stage_48`、`legacy_stage_50` 到 `legacy_stage_59`、`legacy_stage_61` 到 `legacy_stage_62`、`legacy_stage_64`/`66`/`69`/`70`/`72`/`73`/`76`/`77`/`78`/`83`/`86`/`89`、以及 `legacy_stage_91`/`93`-`97`/`99`/`101`-`105`/`107`/`109`/`111`-`112`/`114`-`116`/`119`-`120`、`legacy_stage_122`/`127`-`128`/`130`-`131`/`134`-`137`/`141`/`143`-`148`/`152`/`156`-`158`、`legacy_stage_165`/`172`/`174`-`176`/`178`-`180`/`182`/`186`/`189`/`194`/`196`-`197`/`201`-`202`/`204`-`207`、`legacy_stage_209`-`210`/`214`-`222`/`224`-`231`/`233`、`legacy_stage_234`-`237`/`239`/`242`-`245`/`249`-`252`/`257`-`259`/`263`-`266`、`legacy_stage_268`-`269`/`272`/`276`-`277`/`279`-`281`/`283`/`285`/`287`-`288`/`293`/`295`/`297`-`302` 已接进 app 可玩入口。`legacy_stage_7` 暂不接入，因为其同格部署碰撞属于 `all_units_referenced`，缺少可证明的 opening/deferred 选择规则。`legacy_stage_80` 暂不接入，因为当前默认玩家三人 id 与旧敌方 actor id 发生重复 `pre` spawn/faction 冲突，现已由 native-content duplicate-spawn gate fail-closed。但旧作玩家军来自交互布阵，关卡脚本只给敌/友部署，故通用 generator 目前固定放入刘备/关羽/张飞三人；`LegacySkillMapper` 仍只给全员 `skill_1` 普攻。这是可玩 promotion 的保守默认，不是旧作完整布阵/技能还原。 | 做批量 promotion 前：引入显式 player deployment policy（按剧情/存档/作者配置），并把真技能 importer 扩出普攻以外的效果/战法。Stage 7 需先厘清 `ScriptSetActorVisible` mode/list 语义或引入人工审核的部署策略。Stage 80 需先解决默认玩家 id 与旧 actor id 的映射冲突。 |

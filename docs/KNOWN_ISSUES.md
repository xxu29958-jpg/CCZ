...
| 项 | 说明 | 触发 / 前置 |
|---|---|---|
| **战役驱动层（content → 运行时）—— 战斗 + intro 过场已闭环** | **战斗侧端到端已落地**：`CampaignAssembler` 把 `NativeContent` 装配成 `BattleSetup`，`:app → :native-content` 作组合根，`CampaignContent` 从 bundled JSON 解码，`DemoBattle` 是装配产物薄访问器，mid-trigger `tick` 已接进 `BattleReducer`。**intro 过场内容化已落地**：`events.portrait_subjects` 显式声明非战斗 portrait 主体（如 `cao_cao`），`demo_intro` RScript 进 `campaign.json`，`DemoScenario` 只从 `CampaignContent.introScript()` 取脚本；`ContentValidator` 校验 portrait 引用为 unit 或 portrait_subject，未知/碰撞 fail-closed。**剩余**：接 `SaveLoader`（战斗回放，**S7 caveat** 见下行）/`ScenarioReplayer`（过场回放）。 | 后续片：replay 接线（god-object 风险区，先出边界设计）。 |

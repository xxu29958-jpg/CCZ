...
plus authored Daxingshan intro), `legacy_stage_2` -> `PromotedStageRuntimes.QuyangSiege`, and `legacy_stage_3`
-> `PromotedStageRuntimes.ShimenAttack`, and `legacy_stage_4` -> `PromotedStageRuntimes.SishuiPassOne`
(generated native packs, no authored intros yet).

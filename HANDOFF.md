...
> 状态：引擎实现阶段。自主扫荡持续 —— P2 引擎半 + P3 事件系统引擎侧 + P1 内容 loader（tables）+ Save/Replay 信封 + 事件脚本 JSON 解码（R/S 多态 op）+ 内容→战斗装配（reserves）+ R 剧本引用校验 + R 剧本 ScenarioRunner（含 Choice 续跑）+ Save on-disk codec + Save 命令完整性 均**已合并** main，CI 全绿，**无在途任务**。

## 当前真实状态

- `main` 顶部为 Save 命令完整性片（合并后），本地 == 远端、工作树干净。CI（`.github/workflows/ci.yml`）每 push/PR 跑全套门。test=145。已合并 PR：#3 P2 合法性、#4 P3a 胜负、#5 P3b 触发器、#6 P1 loader、#7 Save 信封、#8 事件脚本 JSON 解码、#9 内容→战斗装配 reserves、#10 R 剧本引用校验、#11 R 剧本 ScenarioRunner、#12 ScenarioRunner Choice 续跑、#13 Save on-disk codec，及 Save 命令完整性 PR。
...
- ~~R 剧本 scenario 流执行（scenario runner）~~ **已落地**（`ScenarioRunner` 确定性 PC 执行 + 控制流消化 + 演出 op 产出 + 循环预算防护）。~~`Choice` 玩家选择 = 可回放输入~~ **已落地**（本片：`run(.., choices)` 用选择索引序列驱动 Choice，应用 setVars + goto，耗尽/越界 fail-safe 暂停；R 剧本 `(script,vars,choices)` 确定回放）。剩余：把 scenario choices 序列接进 `SaveEnvelope` 存档 / 跨会话回放（与战斗 command 序列并列的第二输入轴）—— 触及回放契约，宜独立片。
- ~~Save on-disk codec~~ **已落地**（本片：`SaveCodec` `SaveEnvelope`↔JSON，shape + enum fail-closed，round-trip 全字段保真）。~~命令完整性优雅拒绝~~ **已落地**（`SaveLoader.commandIntegrity`：信封命令引用缺失 unit/skill → `CORRUPT_COMMAND` 优雅拒，replay 前）。剩余：①§Write & Convert Safety 原子写机器门（IO 层，game-core 纯逻辑不碰文件系统，待 IO 边界 / `:app`）；②scenario choices 序列接进 `SaveEnvelope`（与战斗 command 并列第二回放轴）。

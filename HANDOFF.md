...
> 状态：引擎实现阶段。自主扫荡持续 —— P2 引擎半 + P3 事件系统引擎侧 + P1 内容 loader（tables）+ Save/Replay 信封 + 事件脚本 JSON 解码（R/S 多态 op）+ 内容→战斗装配（reserves）+ R 剧本引用校验 + R 剧本 ScenarioRunner（含 Choice 续跑）+ Save on-disk codec + Save 命令完整性 + scenario 进存档 均**已合并** main，CI 全绿，**无在途任务**。

## 当前真实状态

- `main` 顶部为 scenario 进存档片（合并后），本地 == 远端、工作树干净。CI（`.github/workflows/ci.yml`）每 push/PR 跑全套门。test=147。已合并 PR：#3 P2 合法性、#4 P3a 胜负、#5 P3b 触发器、#6 P1 loader、#7 Save 信封、#8 事件脚本 JSON 解码、#9 内容→战斗装配 reserves、#10 R 剧本引用校验、#11 R 剧本 ScenarioRunner、#12 ScenarioRunner Choice 续跑、#13 Save on-disk codec、#14 Save 命令完整性，及 scenario 进存档 PR。
...
- **game-core（Save/Replay 信封 + on-disk codec）**：`com.ccz.core.save` —— `SaveEnvelope`/`SaveVersions`（六版本轴）+ `SaveLoader`（fail-closed 拒未来 save schema / 规则漂移，折叠已接受命令确定性回放）+ `SaveCodec`（`SaveEnvelope`↔JSON：`@Serializable` `SaveDto` 镜像全树 + `SaveMappers` 双向映射 + strict Json，encodeDefaults 显式存档；坏 JSON / 未知 key / 未知命令 kind / 未知 faction·outcome → `SaveDecodeException`，fail-closed）。三闸（`SaveLoader.load`）：版本（FUTURE_SCHEMA / RULES_MISMATCH，`check`）→ 命令完整性（`commandIntegrity` replay 前校验命令引用 unit/skill 对初始 roster / skill 表可解析，缺失 → `CORRUPT_COMMAND` 优雅拒而非 replay fold 抛异常）→ codec 校验 shape。域类型零序列化注解（DTO 隔离，守规则）。`SaveEnvelope.scenarios`（`ScenarioReplay` = scriptId + choices）= 与战斗 command 并列的第二回放轴，`SUPPORTED_SAVE_SCHEMA_VERSION` 1→2；v1 存档缺 scenarios 字段 → decode 为空（forward-compatible）。scenario 回放执行 + scriptId/choices 对 content 完整性校验、原子写（IO）仍待。
